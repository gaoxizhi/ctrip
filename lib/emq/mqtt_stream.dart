...
// Subscribe to an mqtt topic.
  ...
  Future<MqttClient> _login() async {
    ...
    connectJson = await _getBrokerAndKey();
...
//
  Future _subscribe(String topic) async {
    ...
    // }
    if (topic.runes.length == 0) {
      topic = connectJson['key'];
      log.warning('Input topic was none, use default topic.');
    }
...
// Publish to an (Adafruit) mqtt topic.
  Future<void> publish(String topic, String value) async {
    ...
    if (await _connectToClient() == true) {
      ...
      if (topic.runes.length == 0) {
        topic = connectJson['key'];
        log.warning('Input topic was none, use default topic [$topic].');
      }
